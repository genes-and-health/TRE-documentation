// src/components/SandboxSelector.js
import React, { useState } from 'react';

const SandboxSelector = () => {
    const [url, setUrl] = useState('https://console.cloud.google.com/storage/browser/qmul-production-sandbox-1_greendownloads');

    const handleChange = (event) => {
        const baseUrl = 'https://console.cloud.google.com/storage/browser/qmul-production-sandbox-';
        const value = event.target.value;
        setUrl(`${baseUrl}${value}_greendownloads`);
    };
