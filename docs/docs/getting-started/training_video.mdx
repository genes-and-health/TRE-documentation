...
controls url='https://www.dropbox.com/s/k6h5botp53xk3l6/TRE%20Training-20220928_160450-Meeting%20Recording.mp4?e=1&dl=0'/>    


Please feel free to download the video and watch it at your convenience.
...
* Please let your script run first on the cheap 2 core machine. Regenie has really good multithreading and will be fastest on the 64 core huge machine, but this costs. The 2 core machine is fast enough on a big multicore machine that you can run it just from the command line using a single all-chromosome pgen input file, no need for HPC or splitting by chromosome. Regenie will use all threads available minus 1 by default, so the big 64 core machine will run 63 times faster for the compute steps than the 2 core machine. It is fast enough on a big multicore machine that you can run several phenotypes. Obviously if you want to run 1000 phenotypes you will need to use Google HPC / WDL.

```jsx title="Regenie script example"
